...
def business_login_route():
    ...
    if request.method == 'POST':
        ...
        for biz, admin in biz_accounts.items():
            if (admin.get('username') or '').lower() == username and admin.get('password') == password:
                session.clear()
...
def setup_manager(biz):
    ...
    if password != confirm:
        ...
        return redirect(url_for('manager_settings', biz=biz))
    accounts = bl.load_business_accounts()
    accounts[biz] = {'username': username, 'password': password, 'email': email}
    bl.save_business_accounts(accounts)
...
@app.route('/biz/<biz>/manager-settings/update-email', methods=['POST'])
