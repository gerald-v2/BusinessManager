    ...
    with open(filename, mode='w', newline='') as file:
        ...
        for p_name, details in products.items():
            for v in details.get("variants", []):
                writer.writerow(
                    [v.get("PLU", ""), p_name, details.get("Price"), details.get("Cost", "N/A"), v.get("name"),
                     v.get("units"), v.get("sold", 0)])
        ...
        while True:
            ...
            try:
                ...
                if choice == 1:
                    ...
                    new_plu = prompt_unique_plu(name, variant_name, skip_product=product_name,
                                                skip_variant=variant_name)
...
def add_product(name, product_name):
    ...
    raw_cost = input(f"Cost price — what it costs you to make/buy it ({curr}): ").replace("$", "").replace(sym,
                                                                                                           "").strip()
    try:
    ...
    while is_running:
        try:
            ...
            if option == 1:
                ...
                delete_business(input("Name to delete: ").title())
            elif option == 4:
                search_business(input("Search: ").title())
            elif option == 5:
                count_businesses()
            elif option == 6:
                get_all_business()
            elif option == 7:
                edit_business(input("Edit which business: ").title())
            elif option == 8:
                is_running = False
        except ValueError:
